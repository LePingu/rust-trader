...
impl PrivateApiBuilder {
    ...
    pub fn from_env() -> Result<Self, Error> {
        Ok(Self {
            api_key: Some(env::var("KRAKEN_API_KEY")
                .map_err(|e| Error::Auth(format!("Missing API key: {}", e)))?),
            api_secret: Some(env::var("KRAKEN_API_SECRET")
    ...
    pub fn build(self) -> Result<PrivateApi, Error> {
        Ok(PrivateApi {
            api_key: self.api_key.ok_or_else(|| Error::Auth("API key not set".to_string()))?,
            api_secret: self.api_secret.ok_or_else(|| Error::Auth("API secret not set".to_string()))?,
        })
    }
}
...
impl KrakenRequest for PrivateApi {
    ...
    async fn kraken_request<T: for<'de> Deserialize<'de>>(
        ...
        let response: Value = client.make_request_with_retry(url, headers, post_data, endpoint.to_string())
            .await?;
...
impl KrakenRequest for PublicApi {
    ...
    async fn kraken_request<T: for<'de> Deserialize<'de>>(
        ...
        let response: Value = client.make_get_request_with_retry(url, endpoint.to_string())
            .await?;
